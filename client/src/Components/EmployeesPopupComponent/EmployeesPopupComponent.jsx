...
function EmployeesPopupComponent({ show, userHandler, hideHandler }) {
   const { allUsers, allUsersFetchLoading, allUsersFetchError } = useSelector((state) => state.admin);
            ...
            {!!allUsers && allUsers?.success && allUsers?.users ? (
               <div>
                  {allUsers?.users.map((el) => (
                     <div className="emDiv flex items-center justify-between mb-3" key={el._id} onClick={() => userHandler(el)}>
                        <div className="flex items-center">
                           <div className="user_profile_div shadow me-4">
